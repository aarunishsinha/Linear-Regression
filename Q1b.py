...
def plot1(x_dataf, y_dataf ,Theta, x_train):
    plt.figure()
    y_hat = np.array(np.dot(x_train,Theta))
    plt.title('Linear regression')
    plt.scatter(x_train[:,0][:,None],y_dataf,c='b',label='Data')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.plot(x_train[:,0][:,None], y_hat , '-', linewidth=1, c='r', label='Hypothesis')
    plt.legend()
    plt.savefig(os.path.join(sys.argv[2], 'q1_b.png'),dpi=200)
    # plt.show()
def main(eta):
    ...
    plot1(x_dataf, y_dataf, Theta, x_train)
